...
while True:
    ...
    for i in range(m_ntracks):
        ...
        # else:
        filesize += m_seclen + 4
    if input_file[offset + filesize : offset + filesize + 4] == b"XFIH":
        xfih_size = unpack(
            ">I", input_file[offset + filesize + 4 : offset + filesize + 8]
        )[0]
        filesize += xfih_size + 8
    if input_file[offset + filesize : offset + filesize + 4] == b"XFKM":
        xfkm_size = unpack(
            ">I", input_file[offset + filesize + 4 : offset + filesize + 8]
        )[0]
        filesize += xfkm_size + 8
