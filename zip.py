...
def get_zip_file(url):
    ...
    if file_size <= MAX_STANDARD_ZIP_SIZE:
        ...
        cd_start, cd_size = get_central_directory_metadata_from_eocd64(
            zip64_eocd_record
        )
...
def get_file_size(url) -> int:
    return int(requests.head(url).headers.get("content-length") or 0)
...
def get_central_directory_metadata_from_eocd(eocd):
    return parse_little_endian_to_int(eocd[16:20]), parse_little_endian_to_int(
        eocd[12:16]
    )


def get_central_directory_metadata_from_eocd64(eocd64):
    return parse_little_endian_to_int(eocd64[48:56]), parse_little_endian_to_int(
        eocd64[40:48]
    )
...
def parse_little_endian_to_int(little_endian_bytes):
    return struct.unpack(
        "<i" if len(little_endian_bytes) == 4 else "<q", little_endian_bytes
    )[0]
